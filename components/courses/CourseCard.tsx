    ...
    return (
      ...
      >
        <div className="relative rounded-2xl bg-zinc-900/50 border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-all duration-300 hover:shadow-lg hover:shadow-violet-500/5">
          <div className={`h-36 bg-linear-to-br ${styles.gradient} flex items-center justify-center relative overflow-hidden`}>
            {thumbnail?.asset?.url ? (
              <Image 
            ...
            {isCompleted ? (
              ...
              <div className={`absolute top-3 right-3 px-2.5 py-1 rounded-md text-xs font-semibold uppercase tracking-wide ${styles.badge}`}>
                {displayTier}
              </div>
            )}
