...
export function GatedFallback({ requiredTier }: GatedFallbackProps) {
  ...
  return (
    <div className={`relative rounded-2xl bg-linear-to-br ${gradientMuted} border ${styles.border} p-8 md:p-12 overflow-hidden`}>
      <div className="absolute inset-0 bg-[#09090b]/80" />
      <div className={`absolute top-0 right-0 w-64 h-64 bg-linear-to-br ${gradientMuted} rounded-full blur-[100px] opacity-50`} />

      <div className="relative z-10 max-w-xl mx-auto text-center">
        <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-zinc-800/50 border ${styles.border} mb-6`}>
          ...
          <div className="bg-zinc-900/50 rounded-xl p-6 mb-8 text-left">
            ...
            <ul className="space-y-3">
              {tierFeatures.features.map(feature => (
                ...
                >
                  <CheckCircle2 className={`w-4 h-4 mt-0.5 shrink-0 ${styles.text}`} />
                  {feature}
                </li>
              ))}
            </ul>
