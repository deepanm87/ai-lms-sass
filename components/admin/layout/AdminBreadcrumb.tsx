...
const COURSE_QUERY = defineQuery(
  `*[_type == "course" && _.id match "*" + $baseId][0]{ title }`
)
...
const LESSON_QUERY = defineQuery(`{
  ...
  "module": *[_type == "module" && $baseId in lessons[].ref][0]{ _id, title },
...
export default function AdminBreadcrumb() {
  ...
  const query = 
    documentId && documentType === "course"
      ? COURSE_QUERY
  ...
  if (documentType === "courses") {
    items.push({ href: "/admin/courses", label: "Course" })
    if (documentId) {
    ...
    <Breadcrumb>
      <BreadcrumbList className="text-zinc-500">
        {items.map((item, index) => {
          const isList = index === items.length - 1
          return (
            <span
