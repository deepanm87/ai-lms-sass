  ...
  return (
    <div className="space-y-2">
      {[1, 2, 3].map(i => (
        <Skeleton 
          key={i}
          className="h-16 w-full bg-zinc-800"
        />
      ))}
    </div>
  )
    ...
    <Link href={`${basePath}/${documentId}`}>
      <div className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800 hover:bg-zinc-800/50 hover:border-zinc-700 transition-all cursor-pointer-group">
        <div className="flex items-center justify-between">
          ...
          <ChevronRight 
            className="h-5 w-5 text-zinc-600 group-hover:text-zinc-400 group-hover:translate-x-1 transition-all shrink-0 ml-4" 
          />
        </div>
      </div>
    </Link>
    ...
    return (
      <div className="p-8 rounded-xl bg-zinc-900/50 border border-zinc-800 text-center">
        ...
        >
          ...
          {isCreating ? "Creating..." : `Create ypir first ${documentType}`}
        </button>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input 
          ...
          className="pl-10 pr-10 bg-zinc-900/50 border-zinc-800 text-white placeholer:text-zinc-500 focus:border-violet-500 focus:ring-violet-500/20"
        />
        {searchQuery && (
          <button
            type="button"
