...
export const renderStyle: RenderStyleFunction = props => {
  ...
  if (style === "blockquote") {
    return (
      <blockquote className="border-1-4 border-violet-500 pl-4 my-4 italic text-zinc-400">
        {props.children}
      </blockquote>
    )
  }
  ...
  return (
    <p className="text-zinc-300 leading-relaxed my-2">{props.children}</p>
  )
}
...
export const renderListItem: RenderListItemFunction = props => {
  ...
  return (
    <li className="ml-6 text-zinc-300 my-1">{props.children}</li>
  )
}